...
navigate_to() {
  local url=$1
  local response
  response=$(_POST -d '{"url":"'"${url}"'"}' "${BASE_URL}/url")
  _check_error "$response"
}
...
find_element() {
  ...
  local value=$2
  local response
  response=$(_POST -d "{\"using\":\"$property\", \"value\": \"$value\"}" "${BASE_URL}/element")
...
find_elements() {
  ...
  local value=$2
  local response
  response=$(_POST -d "{\"using\":\"$property\", \"value\": \"$value\"}" "${BASE_URL}/elements")
  _check_error "$response" || return 1
  echo "$response" | jq -r '.value[].ELEMENT'
}
...
find_element_from_element() {
  ...
  local value=$3
  local response
  response=$(_POST -d "{\"using\":\"$property\", \"value\": \"$value\"}" "${BASE_URL}/element/${elementId}/element")
  _check_error "$response" || return 1
  echo "$response" | jq -r '.value.ELEMENT'
}
...
find_elements_from_element() {
  ...
  local value=$3
  local response
  response=$(_POST -d "{\"using\":\"$property\", \"value\": \"$value\"}" "${BASE_URL}/element/${elementId}/elements")
  _check_error "$response" || return 1
  echo "$response" | jq -r '.value[].ELEMENT'
}
...
send_keys() {
  ...
  local value=$2
  local response
  response=$(_POST -d "{\"value\": [\"${value}\"]}" "${BASE_URL}/element/${elementId}/value")
  _check_error "$response"
}
...
send_alert_text() {
  local value=$1
  local response
  response=$(_POST -d "{\"value\": [\"${value}\"]}" "${BASE_URL}/alert/text")
  _check_error "$response"
}
